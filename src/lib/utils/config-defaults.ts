...
export async function createDefaultConfig({ userId, envOverrides = {} }: DefaultConfigOptions) {
  ...
  const defaultConfig: typeof configs.$inferInsert = {
    id: configId,
    userId,
    ...
    scheduleConfig: {
      ...
      autoMirror: false,
      lastRun: undefined,
      nextRun: scheduleNextRun ?? undefined,
    },
    cleanupConfig: {
      ...
      lastRun: undefined,
      nextRun: cleanupEnabled ? new Date(Date.now() + getCleanupInterval(cleanupRetentionDays) * 1000) : undefined,
    },
    createdAt: new Date(),
    updatedAt: new Date(),
  };
